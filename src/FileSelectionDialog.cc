  ...
  selection_( selection )
{
  ...
  // button layout
  QHBoxLayout *button_layout = new QBoxLayout();     
...
void FileSelectionDialog::_replace( void )
{
  ...
  QList< QListViewItem* > items( list_->QTreeWidget::selectedItems() );
  for( QList< QListViewItem* >::iterator iter = items.begin(); iter != items.end(); iter++ )
